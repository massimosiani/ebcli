...
def supported_docker_installed():
    ...
    try:
        return compareVersions(SUPPORTED_DOCKER_V, commands.version())
...
def compareVersions(expected, actual):
    actuals = actual.split('.')
    expecteds = expected.split('.')
    try:
        return (int(actuals[0]) > int(expecteds[0])) \
            or (int(actuals[0]) == int(expecteds[0]) and int(actuals[1]) > int(expecteds[1])) \
            or (int(actuals[0]) == int(expecteds[0]) and int(actuals[1]) == int(expecteds[1]) and int(actuals[2]) >= int(expecteds[2]))
    except (ValueError):
        return False
